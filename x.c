...
xloadalpha(void)
{
	float const usedAlpha = focused ? alphafocused : alphaunfocused;
	if (opt_alpha) alphafocused = strtof(opt_alpha, NULL);
...
xloadcols(void)
{
	...
	if (dc.collen) // cannot die, as the color is already loaded.
		xloadcolor(focused ? bgfocused : bgunfocused, NULL, &dc.col[defaultbg]);
...
main(int argc, char *argv[])
{
	...
	case 'A':
		...
		break;
	case 'B':
		alphafocused = atof(EARGF(usage()));
		break;
	case 'b':
		alphaunfocused = atof(EARGF(usage()));
		break;
	case 'c':
